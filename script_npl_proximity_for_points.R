...
# point source npl facilities V1.0
...
get_npl_facility_proximity_for_points <- 
  function(receptor_filepath = NULL,
    ...
    if(is.null(NPL_STATUS)){
      logr::put(stringr::str_c("No NPL_STATUS to filter out the dataset."),
                console = print_log_to_console)
    }else {
      logr::put(stringr::str_c("NPL_dataset filter by NPL_STATUS :", NPL_STATUS))
      source_aircraft_sf <- source_aircraft_sf[source_aircraft_sf$NPL_STATUS != NPL_STATUS, ]
    ...
    if(sf::st_crs(source_aircraft_sf) != projection_crs) {
      ...
      source_aircraft_sf <- st_as_sf(source_aircraft_sf, coords = c("LONGITUDE","LATITUDE"), crs = 4326)
...
check_point_receptor_format <- function(receptor = NULL, 
  ...
  #****currently this code block is not in use
  ...
  # Currently this block is not in use (variable_year)
